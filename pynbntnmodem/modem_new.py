...
from pyatcommand.common import AT_TIMEOUT, dprint
...
class NbntnBaseModem(AtClient, ABC):
    """Base class for a NB-NTN modem abstraction."""
    ...
    def __init__(self, **kwargs) -> None:
        ...
        """
        atclient_keys = {'port', 'baudrate', 'timeout',
                           'autobaud', 'retry_timeout', 'retry_delay',
                           'echo', 'verbose', 'crc', 'autoconfig', 'auto_crc',
                           'command_timeout'}
        kwargs['baudrate'] = kwargs.pop('baudrate', 115200)
        command_timeout = kwargs.pop('command_timeout', None)
        if isinstance(command_timeout, (float, int)):
            self._command_timeout = command_timeout
        if self._command_timeout is not None:
            kwargs['command_timeout'] = self._command_timeout
        super().__init__(**{k: kwargs.pop(k) for k in atclient_keys
                            if k in kwargs})
        self._version: str = ''
        self._imsi: str = ''
        self._imei: str = ''
        ...
        self._apn: str = ''
        self.apn = kwargs.pop('apn', '')
        self._udp_server: str = ''
        if 'udp_server' in kwargs:
            self.udp_server = kwargs.pop('udp_server')
        self._udp_server_port: int = 0
        self.udp_server_port = kwargs.pop('udp_server_port', 0)
        ...
        for step in ntn_init:
            ...
            while not step_success:
                try:
                    ...
                    res: AtResponse = self.send_command(at_cmd, timeout=step.timeout)
    ...
    def set_regconfig(self, config: int) -> bool:
        ...
        res = self.send_command(f'AT+CEREG={config}')
        return res.ok
...
class DefaultModem(NbntnBaseModem):
    ...
    def ping_icmp(self, **kwargs):
        raise NotImplementedError('No universal 3GPP standard method exists')
